    ...
    <div class="row flex-row">
        <div class="col-lg-1-5 primary-sidebar sticky-sidebar">
            ...
            <div class="sidebar-widget widget-category-2 mb-30">
                ...
                @endif
                @if($uniqueAttributes->count())
                    <h5 class="section-title style-1 mb-30 mt-10" style="margin-top:70px !important">Attributi</h5>
                    <ul class="list-group" id="attributes" >
                        @foreach($uniqueAttributes as $parentAttribute)
                            @if($parentAttribute->parent_id == null)
                                <li class="{{ in_array($parentAttribute->id, $filters) ? 'bg-main' : '' }}"
                                    data-id="{{ $parentAttribute->id }}">

                                    <a
                                        class="flex items-center rounded-full  text-[{{$parentAttribute->name}}] hover:bg-amber-600 hover:text-white {{ in_array($parentAttribute->id, $filters) ? 'text-white' : '' }}"><img
                                            src="/assets/imgs/theme/icons/category-1.svg"
                                            alt=""/><strong>{{ ucfirst($parentAttribute->name) }}
                                            ({{ $parentAttribute->products_count }})</strong></a>
                                    <i class="fi-rs-angle-small-right"></i>
                                </li>
                            @endif
                            @if($parentAttribute->childAttributes->count())
                                <div class="list-second-level" data-id="{{ $parentAttribute->id }}" style="display:none"
                                     wire:ignore.self>
                                    @foreach($parentAttribute->childAttributes as $attribute)
                                        <li class="{{ in_array($attribute->id, $filters) ? 'bg-main' : '' }} justify-content-start"
                                            style="margin-left:20px !important"
                                            data-id="{{ $attribute->parent_id }}"  wire:key="btn_{{ $attribute->id }}" wire:click="setAttribute({{ $attribute->id }})">

                                            <a wire:click="$emit('filterByAttribute', {{ $attribute->id }})"
                                               wire:ref="search-box"
                                               class="flex items-center text-gray-400 rounded-full  hover:bg-amber-600 hover:text-white {{ in_array($attribute->parent_id, $filters) ? 'text-white' : '' }}">
                                                <img src="/assets/imgs/theme/icons/category-2.svg"
                                                     alt=""/> {{ ucfirst($attribute->name) }}
                                            </a>
                                        </li>
                                    @endforeach
                                </div>
