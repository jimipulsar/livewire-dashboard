        ...
        <div x-data="{ notificationOpen: false }" class="relative">
            <button @click="notificationOpen = ! notificationOpen" class="flex mx-4 text-gray-600 focus:outline-none">
                @if(getNotifications()->count())
                    <span class="fa-stack fa-1x" data-count="{{getNotifications()->count()}}">
                     <svg class="h-6 w-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            ...
                            <a href="{{ route('adminOrders.show',[ $notify->order_id]) }}"
                               class="flex items-center px-4 py-3 text-gray-600 hover:text-white hover:bg-blue-900 -mx-2">
                                ...
                                <p class="text-sm mx-2">
                                    {{__('Richiesta')}} <span
                                        class="font-bold">{{__('nuovo ordine')}}</span>
                            ...
                            @if($key == 'email_subscription')
                                ...
                                <a href="{{ route('subscribers.index') }}"
                                   class="flex items-center px-4 py-3 text-gray-600 hover:text-white hover:bg-blue-900 -mx-2">
                                    ...
                                    <p class="text-sm mx-2">
                                        {{__('Nuovo utente iscritto alla ')}}<span
                            ...
                            @if($key == 'billing_name')
                                <a href="{{ route('customers.index') }}"
                                   class="flex items-center px-4 py-3 text-gray-600 hover:text-white hover:bg-blue-900 -mx-2">
                                    ...
                                    <p class="text-sm mx-2">
                                        {{__('Nuovo utente')}} <span
                                            class="font-bold">{{__('registrato')}} </span>
                                        - {{ Carbon\Carbon::parse($notify->created_at)->diffForHumans() }}
                                    </p>
                                </a>
                            @endif
